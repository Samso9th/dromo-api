...
authRouter.post("/magic-link", validate({ body: magicLinkSchema }), controller.startMagicLink);
...
function provider(req: Request): OAuthProvider | null {
  ...
  return (OAUTH_PROVIDERS as string[]).includes(p) ? (p as OAuthProvider) : null;
}

authRouter.get("/oauth/:provider", (req, res, next) => {
  const p = provider(req);
  if (!p || !configuredProviders.has(p)) return next(badRequest("Unsupported OAuth provider"));
  passport.authenticate(p, { session: false, scope: OAUTH_SCOPES[p] })(req, res, next);
});
  ...
  passport.authenticate(
    ...
    async (err: unknown, maybeUser: unknown) => {
      try {
        ...
        if (err || !user) return res.redirect(`${env.APP_URL}/login?error=oauth`);
