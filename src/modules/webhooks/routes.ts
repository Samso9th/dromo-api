...
import { handleDubuEvent, handleStripeEvent } from "@/services/payments.service";
...
webhooksRouter.post("/stripe", async (req: Request, res: Response) => {
  if (!stripe || !env.STRIPE_WEBHOOK_SECRET) {
    res.status(503).json({ error: { code: "stripe_unavailable", message: "Stripe not configured" } });
    return;
  }
