...
export const ACTION_TOKENS: Record<GenAction, { in: number; out: number; floor: number }> = {
...
export function creditsForRaw(rawUsd: number, action: GenAction): number {
  return Math.max(ACTION_TOKENS[action].floor, Math.ceil((rawUsd * MARKUP) / CREDIT_USD));
}
...
export function estimateCredits(
  ...
  const raw = Number(pricing.inputPrice) * t.in + Number(pricing.outputPrice) * t.out;
...
export const RETRY_LIMIT: Record<Plan, number> = { free: 0, pro: 2, premium: 5 };
