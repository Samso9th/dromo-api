...
export function assertModelAllowed(tier: ModelTier, plan: Plan, name: string): void {
  if (!modelUnlocked(tier, plan)) {
    throw forbidden(`${name} requires the ${requiredPlanForTier(tier)} plan or higher`);
  }
}
...
export function assertTemplateAllowed(templateId: string, plan: Plan): void {
  ...
  if (!planAtLeast(plan, min)) throw forbidden(`That template requires the ${min} plan or higher`);
}
...
export async function assertConcurrentAvailable(userId: string, plan: Plan): Promise<void> {
  const limit = CONCURRENT_LIMIT[plan];
  if (!Number.isFinite(limit)) return;
  const active = await GenerationSession.count({ where: { userId, status: "active" } });
  if (active >= limit) {
    throw forbidden(
...
export async function assertQaAvailable(sessionId: string, plan: Plan): Promise<void> {
  const used = await ChatMessage.count({ where: { sessionId, role: "user" } });
  if (used >= QA_LIMIT[plan]) {
    throw forbidden(`You've used all ${QA_LIMIT[plan]} Q&A questions for this session.`);
  }
}
