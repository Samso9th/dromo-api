...
const para = (text: string) =>
  new Paragraph({ children: [new TextRun({ text, size: BODY })] });

function buildDoc(data: ResumeData): Document {
  const h = data.header;
  const contact = [h.email, h.phone, h.github, h.linkedin, h.website]
    .filter(Boolean)
    .join("   |   ");
      ...
      children.push(
        leftRight(
          [
            ...
            ...(w.companyUrl
              ? [new TextRun({ text: "   " + w.companyUrl, size: SMALL })]
              : []),
          ],
          [
            new TextRun({
              text: `${w.period.start} – ${w.period.end}`,
              bold: true,
              size: BODY,
            }),
          ],
        ...
        leftRight(
          ...
          [
            new TextRun({
              text: w.location ?? w.locationType,
              italics: true,
              size: BODY,
            }),
          ],
      ...
      children.push(
        leftRight(
          [
            ...
            ...(p.url
              ? [new TextRun({ text: "   " + p.url, size: SMALL })]
              : []),
          ],
          [
            new TextRun({
              text: `${p.period.start} – ${p.period.end}`,
              bold: true,
              size: BODY,
            }),
          ],
    ...
    for (const e of data.extracurriculars) {
      children.push(
        leftRight(
          ...
          [
            new TextRun({
              text: `${e.period.start} – ${e.period.end}`,
              bold: true,
              size: BODY,
            }),
          ],
    ...
    for (const ed of data.education) {
      children.push(
        leftRight(
          ...
          [
            new TextRun({
              text: `${ed.period.start} – ${ed.period.end}`,
              bold: true,
              size: BODY,
            }),
          ],
...
export const resumeDocxBuffer = (data: ResumeData): Promise<Buffer> =>
  Packer.toBuffer(buildDoc(data));
